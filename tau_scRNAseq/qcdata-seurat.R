...
# This R script is only comparing one H1 sample to one KO sample. My computer does not have enough RAM to do the side by side for both samples
...
# ** THIS VERSION OF THE SCRIPT IS NOW ANALYZING THE OTHER TWO FILES

setwd("~/todd_labwork/tau_singlecell/prelimQC")
getwd()

KO <- Read10X_h5("WTCHG_518592_91.h5", ensg.names = FALSE)
H1 <- Read10X_h5("WTCHG_518592_92.h5", ensg.names = FALSE)
...
KO <- FilterCells(KO, subset.names = c("nGene", "nUMI", "percent.mito"), low.thresholds = c(200, -Inf, -Inf), high.thresholds = c(3400, 30000, 0.05))
KO_post <- VlnPlot(object = KO, features.plot = c("nGene", "nUMI", "percent.mito"))
KO_post_l <- length(KO@meta.data$nGene)

H1 <- FilterCells(H1, subset.names = c("nGene", "nUMI", "percent.mito"), low.thresholds = c(200, -Inf, -Inf), high.thresholds = c(3400, 30000, 0.05))
...
FeaturePlot(object = combined, features.plot = c("Nupr1", "Gcg", "Ins1", "Sst","S100a4","Ppy","Plvap","Cd74", "Ctrb1"), min.cutoff = "q9", cols.use = c("lightgrey", "blue"), pt.size = 0.5)

# Labelling the graph with the new identities

new.ident <- c("beta1", "alpha", "beta2", "delta", "beta3", "pp", "endothelial", "leukocytes", "exocrine","lympho?")
for (i in 0:9) {
...
TSNEPlot(combined, do.label = T, pt.size = 0.5)
...
combined@ident <- factor(combined@ident, levels = (c("beta1", "alpha", "beta2", "delta", "beta3", "pp", "endothelial", "leukocytes","exocrine")))
